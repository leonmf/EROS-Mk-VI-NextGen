...
static void Command_RecordSettingsResult(byte action, bool ok, int errorCode = 0)
{
  g_settingsLastAction = action;
  g_settingsLastOk = ok;
  g_bridgeSettingsLastError = errorCode;
...
// Persistent settings save/load is intentionally not implemented on M4.
// M7 owns storage in the split-core architecture. These handlers remain only
// to make accidental legacy commands fail safely instead of touching storage.
static bool Command_RejectM4SettingsSave()
{
  Command_RecordSettingsResult(EROS_SETTINGS_ACTION_SAVE, false, -41001);
  return false;
}

static bool Command_RejectM4SettingsLoad()
{
  Command_RecordSettingsResult(EROS_SETTINGS_ACTION_LOAD, false, -41002);
  return false;
}
...
void Command_Execute(const EROS_Command & command)
{
  switch (command.type)
  {
    ...
    case EROS_CMD_REQUEST_SETTINGS_SAVE:
      Command_RejectM4SettingsSave();
      break;

    case EROS_CMD_REQUEST_SETTINGS_LOAD:
      Command_RejectM4SettingsLoad();
      break;
