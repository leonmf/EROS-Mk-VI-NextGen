/*
  IOBox_M7_stage9_display_after_ready.ino

  Stage 9 split-core M7 test.
...
static void RunHealthChecks()
{
  ...
  if (nowMs - g_lastStatusPollMs >= 1000)
  {
...
void setup()
{
  ...
  Serial.println();
  Serial.println("M7: stage9 display-after-ready test starting");
  Serial.println("M7: no display startup until M4_READY_FOR_RPC and loopback pass");
  Serial.println("M7: uses stable Command4 transport, no old worker thread");
}
