/*
  IOBox_M7_stage16_perf_metrics.ino
...
static void RunHealthChecks()
{
  ...
  if (nowMs - g_lastStatusPollMs >= 50)
  {
...
void setup()
{
  ...
  Serial.println();
  Serial.println("M7: stage16 perf-metrics display starting");
  Serial.println("M7: no display startup until M4_READY_FOR_RPC and loopback pass");
  Serial.println("M7: uses stable Command4 transport; status screen includes M4 loop and M7 status poll metrics");
}
