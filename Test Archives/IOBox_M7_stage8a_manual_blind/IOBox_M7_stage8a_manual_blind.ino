/*
  IOBox_M7_stage8a_manual_blind.ino

  Stage 8A manual output test.

  Use with IOBox_M4_stage8a_manual_blind.
...
static void RunTests()
{
  if (!g_startedTests)
  {
    ...
    g_startedTests = true;
    Serial.println("M7: ready delay complete, beginning Stage 8A manual-blind test");
...
void setup()
{
  ...
  Serial.println();
  Serial.println("M7: stage8A manual-blind test starting");
  Serial.println("M7: uses known-good Stage 6 M4 transport, no extra M4 status selectors");
  Serial.println("M7: waits for M4_READY_FOR_RPC before RPC.call()");
}
