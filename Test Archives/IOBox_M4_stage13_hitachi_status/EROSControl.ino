...
void Control_Setup(void)
{
  ...
  // STAGE 6 DIAGNOSTIC:
  // Skip QSPI/FAT settings storage entirely. Stage 5 proved setup stalls
  // inside Settings_Begin() on the M4 core when started by M7.
  // Load runtime defaults directly so we can prove the rest of M4 startup,
  // transport binding, command processing, and Control_Task() are safe.
  SerialRPC.println("M4_DIAG: skipping Settings_Begin on M4 split-core baseline");
