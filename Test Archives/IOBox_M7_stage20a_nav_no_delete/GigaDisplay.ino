...
static void ProcessDeferredNavigation()
{
  ...
  // Give LVGL one tick after the event callback before deleting/loading screens.
  if (millis() - g_pendingNavActionSetMs < 5UL) {
    return;
  }
  ...
  if (action == NAV_ACTION_AUTO_SETTINGS_BACK) {
    ...
    // Intermittent Mbed SOS crashes were observed when leaving this screen,
    // likely due to LVGL object lifetime/async-delete timing. Reuse the screen
...
static void GigaDisplay_CreateAutoSettingsScreen()
{
  // Stage 19B: fixed pages instead of scrollable Auto Settings.
