...
static bool g_webEnableRequested = false;
static bool g_webRestartRequested = false;
static bool g_webRestartResponseComplete = false;
static unsigned long g_webRestartResponseCompleteMs = 0;
...
static bool g_webDiagnosticDisconnectComplete = false;
...
".restart{width:100%;padding:14px;border:0;border-radius:11px;background:#287bb0;color:white;font-size:16px;font-weight:700}"
...
"<section class=card><h2>Hitachi</h2><div class=value><span id=hitachi>—</span>%</div></section>"
"<section class='card wide'><h2>Local Touchscreen</h2>"
"<button class=restart onclick=restartLocal()>Restart in Local Mode</button>"
"<div class=sub id=restartMsg>Web Control will close and the touchscreen will restart.</div></section></main>"
"<script>"
...
"const activePollMs=100,hiddenPollMs=2000;let pollTimer=0;"
...
"finally{pollTimer=setTimeout(poll,document.hidden?hiddenPollMs:activePollMs)}}"
"async function restartLocal(){clearTimeout(pollTimer);restartMsg.textContent='Restarting…';"
"try{await fetch('/api/restart',{method:'POST',cache:'no-store'})}catch(e){}}"
"document.addEventListener('visibilitychange',()=>{clearTimeout(pollTimer);pollTimer=setTimeout(poll,document.hidden?hiddenPollMs:50)});"
"poll();</script></html>";
...
void EROSWebServer_Enable()
{
  g_webEnableRequested = true;
}

bool EROSWebServer_IsListening()
{
  return g_webListening && WiFi.status() == WL_CONNECTED;
}

void EROSWebServer_GetAddress(char * buffer, size_t bufferSize)
{
  if (buffer == NULL || bufferSize == 0) {
    return;
  }

  if (!EROSWebServer_IsListening()) {
    snprintf(buffer, bufferSize, "Connecting to %s...", EROS_WIFI_SSID);
    return;
  }
...
{
  if (strncmp(g_webRequest, "POST /api/restart ", 18) == 0) {
    static const char restarting[] = "Restarting in local touchscreen mode";
    g_webRestartRequested = true;
    EROSWeb_BeginResponse(
      200, "OK", "text/plain; charset=utf-8",
      restarting, sizeof(restarting) - 1
    );
  }
  else if (strncmp(g_webRequest, "GET /api/state ", 15) == 0) {
...
void EROSWebServer_Setup()
{
  ...
  if (!g_webEnableRequested) return;
...
static void EROSWeb_ServiceWiFi()
{
  ...
  if (g_webDiagnosticDisconnectComplete) {
    return;
  }

  if (WiFi.status() == WL_CONNECTED) {
#if EROS_WEB_DIAGNOSTIC_CONNECT_THEN_DISCONNECT
    Serial.println("WEB_DIAG: connected; now disconnecting Wi-Fi permanently");
    EROSWeb_CloseClient();
    WiFi.end();
    g_webListening = false;
    g_webDiagnosticDisconnectComplete = true;
    Serial.println("WEB_DIAG: Wi-Fi disconnected; begin navigation test");
    return;
#endif
...
void EROSWebServer_Task()
{
  ...
  if (g_webRestartResponseComplete &&
      millis() - g_webRestartResponseCompleteMs >= 250UL) {
    Serial.println("WEB: restarting into local touchscreen mode");
    delay(20);
    NVIC_SystemReset();
  }
  ...
  if (g_webResponseOffset >=
    ...
    if (g_webClientState == WEB_CLIENT_SENDING_HEADER) {
      ...
      const bool completedRestartResponse = g_webRestartRequested;
      EROSWeb_CloseClient();
      if (completedRestartResponse) {
        g_webRestartRequested = false;
        g_webRestartResponseComplete = true;
        g_webRestartResponseCompleteMs = millis();
      }
    }
  }
}
