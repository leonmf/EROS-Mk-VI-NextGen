/*
  ...
  EROS MK VI split-core M7 display/supervisor build.
...
static void RunHealthChecks()
{
  ...
  if (nowMs - g_lastLoopbackMs >= 10000)
  {
    ...
    // Runtime health checks must never run the one-second startup retry loop.
    // This function runs in the same thread that services LVGL; a retry loop
    // here makes the display appear locked whenever RPC is slow or unavailable.
    int accepted = Command4(
      EROS_CMD_TRANSPORT_LOOPBACK_PING,
      0,
      (int)g_loopbackRequestId,
      0
    );

    if (accepted == 0)
    {
      Serial.println("M7: runtime loopback command rejected");
    }
  }
}
...
void loop()
{
  ...
  if (g_serialRpcStarted)
  {
    // Commands generated by LVGL callbacks are sent only after
